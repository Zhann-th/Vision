...
// New UI Elements for Sketch to Code
...
let currentImageFiles = []; // Array to store multiple images
let currentObjectURLs = []; // Store URLs to revoke later
let currentTaskType = 'code';
let lastExcelBase64 = null;
let lastExcelFilename = null; // defaults to Sketch to Code
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = event => {
            ...
            img.onload = () => {
                ...
                }, 'image/jpeg', 0.7); // 70% quality JPEG is very small but readable
            };
            img.onerror = reject;
        };
...
function handleFiles(files) {
    for (let i = 0; i < files.length; i++) {
        ...
        if (isImage) {
            ...
            // For documents, show a generic icon block
    ...
    // Always enable extract button if there are files (or raw text will enable it in its own event if needed, but we check on click anyway)
...
// Enable extract button if text is typed
...
extractBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // Compress images in parallel, but leave docs alone
        const processedFiles = await Promise.all(currentImageFiles.map(file => {
            if (file.type.startsWith('image/')) {
                return compressImage(file);
            }
            return Promise.resolve(file); // return raw doc file
        }));
        ...
        if (data.excelBase64 && data.jsonData) {
            // Build an HTML Table from JSON data for preview
            ...
            // Inject code into Live Preview Iframe
            livePreviewFrame.srcdoc = data.text;
            
            // Open a blank window NOW (synchronously) to bypass popup blockers
...
taskOcrBtn.addEventListener('click', () => {
    ...
    generationSettings.style.display = 'none'; // No simple/creative mode for OCR
    btnText.textContent = 'Extract Text';
    btnIcon.textContent = 'text_fields';
    
    // Result Tabs (force code/text view only)
    tabCode.click();
    tabPreview.style.display = 'none';
    openNewTabBtn.style.display = 'none';
    tabCode.textContent = 'Text';
    extractedText.style.fontFamily = "inherit"; // Use default sans-serif font
});
